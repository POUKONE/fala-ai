...
function MultiSelectMenu({name, options, selected, max, onChange, placeholder}:{name:string;options:string[];selected:string[];max?:number;onChange:(values:string[])=>void;placeholder:string}) {
  return <details className="multi-select-menu"><summary>{selected.length ? selected.join(", ") : placeholder}</summary><div className="multi-select-options">{options.map((option)=><label key={option}><input type="checkbox" checked={selected.includes(option)} disabled={!selected.includes(option)&&selected.length>=(max ?? Infinity)} onChange={(event)=>onChange(event.target.checked ? [...selected,option].slice(0,max) : selected.filter((item)=>item!==option))}/><span>{option}</span></label>)}</div>{selected.map((value)=><input key={`${name}-${value}`} type="hidden" name={name} value={value}/>)}</details>;
}
