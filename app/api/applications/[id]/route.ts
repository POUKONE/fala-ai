...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const updated = await getPostgresDb().prepare("UPDATE applications SET status=?, notes=?, next_action_at=?, interview_at=?, updated_at=? WHERE id=? AND user_email=? RETURNING *")
