...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const current = await getPostgresDb().prepare("SELECT * FROM applications WHERE id = ? AND user_email = ?").bind(id,user.email).first<Record<string, unknown>>();
  ...
  await db.prepare("UPDATE applications SET status=?, notes=?, next_action_at=?, interview_at=?, updated_at=? WHERE id=? AND user_email=?")
    .bind(status,notes,nextActionAt,interviewAt,new Date().toISOString(),id,user.email).run();
  const updated = await db.prepare("SELECT * FROM applications WHERE id=? AND user_email=?").bind(id,user.email).first();
...
export async function DELETE(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await context.params;
  const result = await getPostgresDb().prepare("DELETE FROM applications WHERE id = ? AND user_email = ?").bind(id,user.email).run();
