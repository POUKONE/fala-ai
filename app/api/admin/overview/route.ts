...
export async function GET() {
  ...
  return Response.json({ summary:{users:users?.count??0,active7d:active?.count??0,applications:applications?.count??0,newApplications7d:recentApps?.count??0},statuses:statuses.results,recentUsers:recentUsers.results,activity:activity.results });
}
