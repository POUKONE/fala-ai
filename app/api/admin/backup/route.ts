...
export async function GET() {
  ...
  const payload = JSON.stringify({ exportedAt: new Date().toISOString(), externalBackup: external, users: users.results, profiles: profiles.results, applications: applications.results, activity: activity.results, reports: reports.results, roles: roles.results }, null, 2);
