...
export async function GET(){
  ...
  const rows=await getPostgresDb().prepare("SELECT u.email,u.display_name,u.created_at,u.last_seen_at,u.suspended_at,COUNT(a.id) AS applications FROM users u LEFT JOIN applications a ON a.user_email=u.email GROUP BY u.email ORDER BY u.created_at DESC").all<Record<string,unknown>>();
