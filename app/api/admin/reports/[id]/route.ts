...
export async function PATCH(request:Request,context:{params:Promise<{id:string}>}){
  ...
  const updated=await getD1().prepare("UPDATE reports SET status=?,admin_note=?,updated_at=? WHERE id=? RETURNING *").bind(status,String(body.adminNote??"").slice(0,1000),new Date().toISOString(),id).first();
