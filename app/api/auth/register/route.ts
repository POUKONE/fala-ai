...
import { createSession, hashPassword, SESSION_COOKIE } from "../../../email-auth";
...
export async function POST(request: Request) {
  ...
  await db.prepare("INSERT INTO users (email,display_name,created_at,last_seen_at,password_hash,verification_token,consent_version,consented_at) VALUES (?,?,?,?,?,?,?,?)")
    .bind(email, name, now, now, await hashPassword(password), token, CONSENT_VERSION, now).run();
  void sendTransactionalMail({ to: email, subject: "Vérifiez votre adresse Fala AI", html: `<p>Bienvenue sur Fala AI.</p><p><a href="${new URL(`/auth/verify?token=${encodeURIComponent(token)}`, request.url)}">Vérifier mon adresse e-mail</a></p>` }).catch(() => {});
  const session = await createSession(email);
  const response = Response.json({ ok: true, user: { email, displayName: name }, emailVerificationRequired: true, message: "Votre compte est créé. Vérifiez votre adresse e-mail pour finaliser l’activation." });
