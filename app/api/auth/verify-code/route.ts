...
export async function POST(request: Request) {
  ...
  const row = await getPostgresDb().prepare("SELECT verification_token,email_verified_at,password_hash FROM users WHERE lower(email)=lower(?)").bind(email).first<{verification_token:string|null;email_verified_at:string|null;password_hash:string|null}>();
  ...
  if (!await verifyPassword(code, challenge.hash)) { await getPostgresDb().prepare("UPDATE users SET verification_token=? WHERE lower(email)=lower(?)").bind(JSON.stringify({ ...challenge, attempts: challenge.attempts + 1 }), email).run(); return Response.json({ error: "Code incorrect" }, { status: 400 }); }
  await getPostgresDb().prepare("UPDATE users SET email_verified_at=?,verification_token=? WHERE lower(email)=lower(?)").bind(new Date().toISOString(), "verified", email).run();
